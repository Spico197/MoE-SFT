...
class OlmoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, seq_pad_mask=None) -> torch.Tensor:
        ...
        # (num_tokens, topk, num_experts) -> (num_experts, topk, num_tokens)
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts).permute(2, 1, 0)

        gate_load = expert_mask.sum(dim=1).permute(1, 0)
        if seq_pad_mask is not None:
            gate_load[~seq_pad_mask.flatten().bool()] = 0
        ...
        return final_hidden_states, router_logits, gate_load
...
class OlmoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits, gate_load = self.mlp(hidden_states)
        hidden_states = residual + hidden_states

        outputs = (hidden_states, gate_load)
...
@dataclass
class GateLoadMoeModelOutputWithPast(MoeModelOutputWithPast):
...
class OlmoeModel(OlmoePreTrainedModel):
    ...
    def forward(
        ...
        return GateLoadMoeModelOutputWithPast(
...
@dataclass
class GateLoadMoeCausalLMOutputWithPast(MoeCausalLMOutputWithPast):
    gate_load: Optional[Tuple[torch.LongTensor]] = None
