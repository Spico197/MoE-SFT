...
#SBATCH --job-name=llama_moe_uniform_final_static
...
{
    task_name="llama_moe_uniform_final_static"
    model_type="auto"
    ...
    comment="llama-moe 2/8, four type mix, sampling with final static weights, 4 gpus, eval_steps 100, max_eval_steps 5, w/ balance loss, w/ freeze gate, w/ gate noise"
    base_dir="outputs"
    ...
    --rdzv_endpoint $head_node:29522 \
        -m src.core.train \
            --do_train \
            --freeze_gate True \
            --prob_map conf/prob_map/llama_moe_dynamic_final.json \
