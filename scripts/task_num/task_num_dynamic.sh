...
#SBATCH --job-name=llama_moe_dynamic_task_num
...
multi_eval() {
    ...
    python -m src.utils.gpu "5"
    note "bbh device OK - 5"
    ...
    python -m src.utils.gpu "6"
    note "reasoning device OK - 6"
    ...
    python -m src.utils.gpu "7"
    note "mmlu device OK - 7"
...
task_num_job() {
    num_tasks=$1
    task_name="dynamic_task_$num_tasks"
    model_type="auto"
